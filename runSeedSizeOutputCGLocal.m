function [] = runSeedSizeOutputCGLocal(seed,outputMat_1,outputMat_2,numOfInputs,numOfOutputs,numOfGates,numOfCandidateSolutions,mult,preDefinedSize,freqAlternate,L,mutProb,onlyChangeConnections)
...
% EVOLUTION
% The random mutations (always respecting the backward patterning) can be:
...
if(mutProb==0)
    save(['./local_output/BEFORE_TOL_FITTEST_CIRCUIT_SIZE_CG_SEED_' num2str(seed) '_' num2str(sim) '.mat'])
else
    save(['./local_output/BEFORE_TOL_FITTEST_CIRCUIT_SIZE_CG_MUT_SEED_' num2str(seed) '_' num2str(sim) '.mat'])
end
...
outputMat = outputMat_1; %CONSTANT GOAL, NOT VARYING!
while(sim<5*1e4)
...
save(['./local_output/BEFORE_TOL_ALL_SIZE_CG_SEED_' num2str(seed) '.mat'])
